...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        ## YOUR CODE HERE ####
        a_nodesToRoot = self.path_to_root(a)
        b_nodesToRoot = self.path_to_root(b)
        common_ancestor = []
        
        a_node = self.names_to_nodes[a]
        b_node = self.names_to_nodes[b]
        
        if a_node == b_node or a_node.get_parent() == b_node or b_node.get_parent() == a_node:
            t = -1
            r = 0
        else:
            for inode in a_nodesToRoot:
                if inode in b_nodesToRoot:
                    common_ancestor.append(inode) 
            a_dist = a_nodesToRoot.index(common_ancestor[0])
            b_dist = b_nodesToRoot.index(common_ancestor[0])
            t = min(a_dist-1,b_dist-1)
            r = abs(a_dist - b_dist)
        return t,r
    
    # Add a method to retrieve all parents nodes to root from a
    def path_to_root(self,a):
        '''
        Returns a list of members which leads root of member object a.
        ...
        a_node = self.names_to_nodes[a]
        nodesToRoot = []
        while a_node != self.root:
            nodesToRoot.append(a_node)
            a_node = a_node.get_parent()
        if a_node == self.root:
            nodesToRoot.append(self.root)
        return nodesToRoot
