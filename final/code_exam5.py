...
class edx(object):
    ...
    def setGrade(self, grade, course="6.01x"):
        ...
        assert 0<= grade <= 100 and type(grade) == int
        for courseObj in self.myCourses:
            if course == courseObj.courseName:
                courseObj.setGrade(grade)
    ...
    def setPset(self, pset, score, course="6.00x"):
        ...
        assert 0 <= score <= 100 and type(score) == int
        for courseObj in self.myCourses:
            if course == courseObj.courseName:
                courseObj.setPset(pset,score)
